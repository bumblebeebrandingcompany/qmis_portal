...
<body>
    ...
    <div class="form-container">
        ...
        <div id="lead-details-card" class="lead-details-card mt-3" style="display: none;">
            ...
            <div class="row">
                <div class="col-6">
                    <p><strong>Ref:</strong> <a id="ref_num_link" href="#"><span id="ref_num"></span></a></p>
                </div>
                <div class="col-6">
            ...
            $(document).ready(function() {
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            if (response.status === 'found') {
                                ...
                                $('#ref_num_link').attr('href', lead.ref_url || '#');
                                ...
                                $('#otp-container').hide(); // Hide OTP container if lead found
                                $('#form-container').hide(); // Hide form container initially

                                // Set the lead_id in the hidden input field
                                $('#lead_id').val(lead.id);

                                // Update form action to the update route
                                $('#newLeadForm').attr('action', `/walkin/update/${lead.id}`);

                            } else if (response.show_otp_container) {
                                $('#lead-details-card').hide();
                                $('#otp-container').show(); // Show OTP container if no lead found
                                $('#form-container').hide(); // Hide form container initially
                                ...
                                $('#lead-details-card').hide();
                                $('#otp-container').hide();
                                $('#form-container').show(); // Show form container if necessary
                                $('#disclaimer').text(response.message).show();

                                // Reset form action to store if no lead is found
                                // $('#newLeadForm').attr('action', '/walkin/store');
                            }
                        }
                    });
                });
                ...
                $(document).ready(function() {
                    ...
                    $('#otpForm').on('submit', function(e) {
                        e.preventDefault();
                        var otp = $('#otp-input').val(); // Get the OTP value
                        var leadId = $('#lead_id').val(); // Get the lead_id value from the hidden input

                        $.ajax({
                            url: '/walkin/validate-otp', // The endpoint for OTP validation
                            method: 'POST',
                            data: {
                                otp: otp,
                                lead_id: leadId, // Send lead_id for verification
                                _token: '{{ csrf_token() }}' // Add CSRF token for security
                            },
                            success: function(response) {
                                if (response.status === 'valid') {
                                    // OTP is valid, redirect to the update form page
                                    window.location.href = 'walkin/update-form/' + leadId;
                                } else {
                ...
                $('#lead_id').val(response.lead_id); // Set the lead ID after lead creation
