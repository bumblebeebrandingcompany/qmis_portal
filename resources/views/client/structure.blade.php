...
@section('content')
    <div class="container mt-4">
        <p>Thank you for taking the time to submit your details.
            You are on waiting list no. {{@$lead->application->waiting_list}} . You will receive a
            confirmation message once your application has
            been reviewed and accepted.
        </p>
        <p>Meanwhile, explore our brochure, HVCOs,
            blogs, virtual campus, and testimonials here along with thr fee structure and offers.
        </p>
        <p>For any assistance or query, feel free to reach out to us +91 789 789 9389
        </p>
        <h3>Unlock Exclusive Guidance for Your Child’s Future
        </h3>
        <p>Get exclusive access to a carefully curated set of documents to help you make informed decisions for your child's
            education journey.</p>
        <div><span>You can download the Application form</span><a href="{{route('qrdownload', ['no' => @$lead->application->application_no])}}" class="mx-1">Here</a></div>
    ...
    <div class="bottom-container">
        ...
        <a class="schedule-button {{ $lead->parent_stage_id == 30 ? '' : 'disabled' }} text-white"
            style="font-weight: bold;color:#ffffff text-align: center; display: block; justify-content:center;{{ $lead->parent_stage_id != 30 ? 'pointer-events: none; opacity: 0.5;' : '' }}"
            data-toggle="modal" data-target="#scheduleVisitModal">
                ...
                <form action="{{ route('client.storeSiteVisit', ['id' => $lead->id]) }}" method="POST">
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            ...
                            <input type="date" class="form-control custom-input" id="visitDate" name="date"
                                required> <!-- Updated name -->
                        </div>
                        <div class="form-group">
                            <label for="timeSlot" class="custom-text">Available Time Slots:</label>
                            <select class="form-control custom-input" id="timeSlot" required>
                                <!-- Updated name -->
                        ...
                        <div class="form-group">
                            ...
                            <input type="hidden" id="selectedTimeSlot" name="time_slot" value="">
                            <!-- Hidden input to store the selected time slot -->
                        </div>

                        <div class="form-group">
                            <label for="description" class="custom-text">Description:</label>
                            <textarea id="description" class="form-control custom-input" name="description" rows="4"
    ...
    <script>
        document.getElementById('timeSlot').addEventListener('change', function() {
            ...
            const hiddenInput = document.getElementById('selectedTimeSlot'); // Reference to the hidden input
